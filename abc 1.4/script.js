...
// JUMP kk
function spazio(e) {
    if (e.keyCode == 32)  // 32 = barra spaziatrice
        jump();
}

// Jump vecchio (obsoleto)
// function jump(ev) {
//     if (ev.key != " ") return
...
function start(){
    ...
    // fa saltare l'omino
    document.addEventListener('keydown', spazio);
...
function jump() {
    if (omino.classList == "animazione")
        return;
    omino.classList.add("animazione");
    setTimeout(function () {
        omino.classList.remove("animazione");
    }, 400);
}
